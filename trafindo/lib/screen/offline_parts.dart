...
class OfflinePartsReplacementScreen extends StatefulWidget {
  final List<Map<String, dynamic>> offlinePartsData;

  const OfflinePartsReplacementScreen({Key? key, required this.offlinePartsData})
      : super(key: key);

  @override
  _OfflinePartsReplacementScreenState createState() =>
      _OfflinePartsReplacementScreenState();
}

class _OfflinePartsReplacementScreenState extends State<OfflinePartsReplacementScreen> {
  ...
  void initState() {
    ...
    // Listen for changes in offlinePartsData and update the count
    _countController.add(widget.offlinePartsData.length);
  }
  ...
  List<DataRow> _buildDataTableRows() {
    ...
    if (widget.offlinePartsData.isEmpty) {
      ...
      for (var data in widget.offlinePartsData) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Offline Parts Replacement Needed'),
      ),
      body: Column(
        children: [
          StreamBuilder<int>(
            stream: _countController.stream,
            initialData: widget.offlinePartsData.length,
              ...
              return Padding(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text(
                        'Offline Parts Replacement Needed',
                        style: TextStyle(fontSize: 12),
                      ),
                      const SizedBox(
                        height: 5.0,
